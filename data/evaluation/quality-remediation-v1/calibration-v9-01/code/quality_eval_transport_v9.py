"""V9 reasoning grader transport with separately priced, durable API accounting."""
...
from scripts.quality_eval_contract_v9 import request_parts, schema, review_schema, REVIEW_PROMPT, candidate_judgments
...
# Verified 2026-09-19: https://developers.openai.com/api/docs/models/gpt-5.4-mini
...
def request(purpose, prompt, data, contract):
    return {"model": MODEL, "reasoning_effort": "low", "max_completion_tokens": CAPS[purpose],
...
class Ledger:
    ...
    def __init__(self, path, *, historical=LEDGER):
        ...
        self.previous = json.loads((Path(__file__).resolve().parents[1] / "data/evaluation/quality-remediation-v1/calibration-v8-01/api-ledger.json").read_bytes())["calls"]
        self.reload()
    ...
    def reload(self):
        ...
        if self.data["calls"][:len(self.previous)] != self.previous:
            raise BudgetStop("Frozen v8 continuation prefix changed")
