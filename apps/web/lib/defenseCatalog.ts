...
export const productionReadinessCatalog: DefenseCatalogItem[] = [
  {
    ...
    limitations: ["No live identity provider, MFA, conditional-access policy, refresh-token exchange, or provider logout is connected; Phase 57 database enforcement is local evidence, not hosted deployment proof."],
  ...
  },
  {
    ...
    phase: "Phase 57",
    status: "implemented",
    summary: "Adds transaction-scoped tenant context, forced PostgreSQL row-level security, tenant-aware relationship constraints, and direct runtime-role isolation tests.",
    boundary: "Local database policy evidence prevents a missing application filter from exposing another tenant; it is not a connected hosted-database claim.",
    evidence: [
      { label: "Permissions and scope", href: "/algorithm", detail: "Explains the application and database authorization layers." },
      { label: "Defense readiness", href: "/dev-admin/defense-readiness", detail: "Keeps local controls distinct from production and independent validation." },
    ],
    limitations: ["A separately provisioned hosted runtime login and migration owner have not been connected or tested; session tenant settings remain a trusted application boundary."],
    last_verified: "2026-08-28",
  },
  {
    id: "abuse-controls",
    ...
    phase: "Phase 58",
    status: "implemented",
    summary: "Applies identity, tenant, client-risk, operation, concurrency, request-bound, retry, and external-AI admission controls through one shared-store contract.",
    boundary: "Two independent clients passed atomic tests against the free local Redis Compose service; no managed or hosted cache is connected.",
    evidence: [
      { label: "Trust boundary guide", href: "/algorithm", detail: "Keeps authorization, evidence, and abuse-control responsibilities separate." },
      { label: "Defense readiness", href: "/dev-admin/defense-readiness", detail: "Shows measured defense evidence without implying hosted operations." },
    ],
    limitations: ["Hosted Redis TLS/auth, HA, trusted-proxy IP policy, billing reconciliation, production thresholds, alert ownership, and streaming proxy byte caps remain external deployment work."],
    last_verified: "2026-08-28",
  },
  {
    id: "secure-files",
    ...
    phase: "Phase 59",
    status: "implemented",
    summary: "Quarantines PDF originals under opaque tenant keys, validates a strict file envelope, records a forced-RLS lifecycle, and gates pending review behind local scan and bounded parse contracts.",
    boundary: "Safe local fixtures prove rejection and no-index-before-review behavior; fixture scanning and subprocess parsing are not production malware or isolation controls.",
    evidence: [
      { label: "Trust boundary guide", href: "/algorithm", detail: "Explains why only authorized, indexed evidence can enter retrieval and generation." },
      { label: "Defense readiness", href: "/dev-admin/defense-readiness", detail: "Keeps local implementation evidence separate from hosted and independently validated controls." },
    ],
    limitations: ["Synthetic/non-sensitive PDF only; hosted encrypted object storage, a real malware scanner, a connected isolated worker, retention scheduling, and dependency/image scanning remain deployment work."],
    last_verified: "2026-08-28",
  },
  {
    id: "secrets-privacy",
    ...
    phase: "Phase 60",
    status: "implemented",
    summary: "Adds mounted-file and fail-closed managed secret boundaries, placeholder/TLS startup guards, recursive log and telemetry redaction, prompt fingerprints, and executable local retention/hold controls.",
    boundary: "Local tests prove content minimization and provider contracts; they do not prove managed identity, encrypted centralized storage, backup deletion, or production operations.",
    evidence: [
      { label: "Defense readiness", href: "/dev-admin/defense-readiness", detail: "Keeps local defense evidence separate from connected production dependencies." },
      { label: "Algorithm boundaries", href: "/algorithm", detail: "Shows why prompt, memory, and authorized evidence remain request data rather than log evidence." },
    ],
    limitations: ["Managed secrets/workload identity, centralized encrypted logs, tenant export/deletion, backup expiry, automatic rotation, and provider data-use configuration remain external or operational work."],
    last_verified: "2026-08-28",
  },
  {
    ...
    phase: "Phase 61",
    status: "implemented",
    summary: "Emits tenant-scoped, privacy-safe events into a locally verified hash chain, evaluates documented thresholds, and exercises six incident runbook contracts.",
    boundary: "The local sink proves event-to-alert wiring and tamper detection; it is runtime-rewritable and is not a staffed SIEM, pager, or immutable audit archive.",
    evidence: [
      { label: "Security monitoring", href: "/dev-admin/security-monitoring", detail: "Shows the active tenant's content-free events, local alerts, chain status, and threshold catalog." },
      { label: "Defense readiness", href: "/dev-admin/defense-readiness", detail: "Keeps local monitoring evidence separate from production operations." },
    ],
    limitations: ["A live monitoring/SIEM destination, named on-call owner, escalation policy, immutable external store, and notification channel remain external decisions."],
    last_verified: "2026-08-28",
  },
  {
    id: "penetration-test",
