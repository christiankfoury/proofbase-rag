  ...
  return (
    <Shell>
      <PageHeader
        ...
        description={`Inspect available per-question benchmark rows for ${runName}. Some historical runs only have summary metrics.`}
      />
      <section className="grid gap-4 md:grid-cols-4">
        <Card padding="compact">
          <p className="text-sm font-medium text-steel">Run</p>
          <p className="mt-2 font-semibold text-ink">{runName}</p>
        </Card>
        <Card padding="compact">
          <p className="text-sm font-medium text-steel">Phase</p>
          <p className="mt-2 font-semibold text-ink">{typeof data.run?.phase === "string" ? data.run.phase : "n/a"}</p>
        </Card>
        <Card padding="compact">
