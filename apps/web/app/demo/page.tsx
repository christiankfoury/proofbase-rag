      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <Card tone="good">
          ...
          <ol className="mt-6 space-y-4">
            {demoSteps.map((step) => (
              <li key={step.number} className="rounded-md border border-stone-200 bg-stone-50 p-4">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="flex min-w-0 gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-moss bg-moss-soft text-sm font-bold text-moss-dark">
                      {step.number}
                    </span>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="font-semibold text-ink">{step.title}</p>
                        <span className="badge-neutral">{step.outcome}</span>
                      </div>
