...
- Security-assessment preparation now includes a threat model, rules of engagement, OWASP coverage map, non-root images, local prechecks, and finding/retest workflow. It remains `Independent validation required`; current CVE/SBOM tooling, network DAST, hosted-provider tests, and any independent engagement are open.
- Ongoing release controls now bind 18 deterministic checks, hard and quality gates, finding state, human review, rollback/monitoring readiness, corpus/policy/suite provenance, and sealed-suite custody to an exact runtime commit. Local portfolio controls pass; production is blocked by human review, live monitoring/on-call, hosted availability evidence, and independent validation.
