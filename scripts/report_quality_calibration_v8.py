"""Replay frozen v8 calibration from saved requests and responses; no API calls."""
import argparse
...
from scripts.quality_eval_calibration_v8 import FOLDER, SUITE, AUDITS, VALIDATION, digest, compare, CODE
from scripts.quality_eval_contract_v8 import dimensions, review_schema
from scripts.quality_eval_transport_v8 import initial_requests, review_request, parsed, Ledger
from scripts.quality_eval_transport import INPUT_RATE, OUTPUT_RATE
...
def replay(attempt="v8-01"):
    if attempt != "v8-01":
        raise ValueError("This frozen reporter handles only v8-01")
    ...
    for name in CODE:
        ...
        if (ROOT / "scripts" / name).read_text(encoding="utf-8") != (folder / "code" / name).read_text(encoding="utf-8"):
            raise ValueError("Replay requires the frozen v8 evaluator source")
    ...
    for case in read(AUDITS)["cases"]:
        ...
        if review != saved["review"] or matched != saved["matched"]:
            raise ValueError("Audit replay mismatch")
        audit_rows.append({"id": case["id"], "matched": matched, "review": review})
...
if __name__ == "__main__":
    ...
    result = replay()
    path = FOLDER / "calibration-v8-report.json"
    if args.write:
        if path.exists():
