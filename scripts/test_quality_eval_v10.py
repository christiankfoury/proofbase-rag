...
from scripts.quality_eval_contract_v10 import dimensions
from scripts import quality_eval_transport_v10 as transport


class V10Tests(unittest.TestCase):
    def ledger(self, folder):
        path = Path(folder) / "ledger.json"
        source = Path(__file__).resolve().parents[1] / "data/evaluation/quality-remediation-v1/calibration-v9-01/api-ledger.json"
    ...
    def test_preserves_v7_and_v8_history(self):
        with tempfile.TemporaryDirectory() as temp:
            ledger = self.ledger(temp)
            self.assertEqual(len(ledger.data["calls"]), 1546)
            ledger.data["calls"][-1]["charged_usd"] = "0"
            ledger.path.write_text(json.dumps(ledger.data))
            with self.assertRaisesRegex(transport.BudgetStop, "v9 continuation prefix"):
