"""Replay frozen v7 calibration from saved requests and responses; no API calls."""
import argparse
...
from scripts.quality_eval_calibration import FOLDER, SUITE, AUDITS, VALIDATION, digest, compare, CODE
from scripts.quality_eval_contract import dimensions, review_schema
from scripts.quality_eval_transport import initial_requests, review_request, parsed, Ledger, INPUT_RATE, OUTPUT_RATE
...
def replay(attempt="v7-01"):
    if attempt != "v7-01":
        raise ValueError("This frozen reporter handles only v7-01")
...
if __name__ == "__main__":
    ...
    result = replay()
    path = FOLDER / "calibration-v7-report.json"
    if args.write:
        if path.exists():
