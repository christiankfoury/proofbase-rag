"""Version 12 development calibration; frozen v7 runner and evidence are unchanged."""
...
from scripts.quality_eval_contract_v12 import BEHAVIORS, DIMENSIONS, VERSION, dimensions
from scripts.quality_eval_preflight import budget_audit
from scripts.quality_eval_transport_v12 import MODEL, APPROVED_CEILING, AUTHORIZATION, Ledger, case_bound, grade_case, review_request, reserve, parsed, BudgetStop
...
CODE = ("quality_eval_contract.py", "quality_eval_transport.py", "quality_eval_preflight.py", "quality_eval_contract_v8.py", "quality_eval_contract_v12.py", "quality_eval_transport_v12.py", "quality_eval_calibration_v12.py")
...
def execute(attempt, create, ledger, *, suite_path=SUITE, validation_path=VALIDATION, audit_path=AUDITS, folder=FOLDER):
    ...
    try:
        ...
        for case in load_audits(audit_path)["cases"]:
            ...
            exact = bool(review and all(review[key] == ("dispute" if key in case["expected_disputes"] else "agree") for key in (*DIMENSIONS, "forbidden_assertion")))
            path = out / (case["id"] + "-review.json")
            write_json_atomic(path, {"case_id": case["id"], "review": review, "matched": matched, "exact_match": exact, "grading_error": error})
            manifest["review_rows"][case["id"]] = {"sha256": digest(path), "matched": matched, "exact_match": exact}
            write_json_atomic(out / "manifest.json", manifest)
