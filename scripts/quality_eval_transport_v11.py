"""V11 reasoning grader transport with separately priced, durable API accounting."""
...
from scripts.quality_eval_contract_v11 import request_parts, schema, review_schema, REVIEW_PROMPT, candidate_judgments
...
class Ledger:
    ...
    def __init__(self, path, *, historical=LEDGER):
        ...
        self.previous = json.loads((Path(__file__).resolve().parents[1] / "data/evaluation/quality-remediation-v1/calibration-v10-01/api-ledger.json").read_bytes())["calls"]
        self.reload()
    ...
    def reload(self):
        ...
        if self.data["calls"][:len(self.previous)] != self.previous:
            raise BudgetStop("Frozen v10 continuation prefix changed")
