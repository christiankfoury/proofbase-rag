"""Replay frozen v10 calibration from saved requests and responses; no API calls."""
import argparse
...
from scripts.quality_eval_calibration_v10 import FOLDER, SUITE, AUDITS, VALIDATION, digest, compare, CODE
from scripts.quality_eval_contract_v10 import dimensions, review_schema, DIMENSIONS
from scripts.quality_eval_transport_v10 import initial_requests, review_request, parsed, Ledger
from scripts.quality_eval_transport_v10 import INPUT_RATE, OUTPUT_RATE, MODEL
...
def replay(attempt="v10-01"):
    if attempt != "v10-01":
        raise ValueError("This frozen reporter handles only v10-01")
    ...
    for name in CODE:
        ...
        if (ROOT / "scripts" / name).read_text(encoding="utf-8") != (folder / "code" / name).read_text(encoding="utf-8"):
            raise ValueError("Replay requires the frozen v10 evaluator source")
    ...
    for row in ledger.data["calls"][len(ledger.prefix["calls"]):]:
        ...
        rates = {"gpt-4.1-2025-04-14": (Decimal("2"), Decimal("8")), MODEL: (INPUT_RATE, OUTPUT_RATE)}
...
if __name__ == "__main__":
    ...
    result = replay()
    path = FOLDER / "calibration-v10-report.json"
    if args.write:
        if path.exists():
