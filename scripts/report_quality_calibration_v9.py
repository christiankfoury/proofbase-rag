"""Replay frozen v9 calibration from saved requests and responses; no API calls."""
import argparse
...
from scripts.quality_eval_calibration_v9 import FOLDER, SUITE, AUDITS, VALIDATION, digest, compare, CODE
from scripts.quality_eval_contract_v9 import dimensions, review_schema, DIMENSIONS
from scripts.quality_eval_transport_v9 import initial_requests, review_request, parsed, Ledger
from scripts.quality_eval_transport_v9 import INPUT_RATE, OUTPUT_RATE, MODEL
...
def replay(attempt="v9-01"):
    if attempt != "v9-01":
        raise ValueError("This frozen reporter handles only v9-01")
    ...
    for name in CODE:
        ...
        if (ROOT / "scripts" / name).read_text(encoding="utf-8") != (folder / "code" / name).read_text(encoding="utf-8"):
            raise ValueError("Replay requires the frozen v9 evaluator source")
...
if __name__ == "__main__":
    ...
    result = replay()
    path = FOLDER / "calibration-v9-report.json"
    if args.write:
        if path.exists():
