...
MAX_BUDGET = 2.0
PRICES = {"gpt-4.1-2025-04-14": (2.0, 8.0), "gpt-4.1-mini": (0.4, 1.6), "gpt-4.1-mini-2025-04-14": (0.4, 1.6),
...
class Ledger:
    def __init__(self, path: Path, limit: float = MAX_BUDGET):
        if not 0 < limit <= MAX_BUDGET:
            raise ValueError("Budget must be in (0, 2.0]")
        self.path, self.limit = path, limit
        if not path.exists():
            raise ValueError("Initialize an authorized ledger with the historical prefix first")
        self.lock = threading.Lock()
        self.data = json.loads(path.read_text(encoding='utf-8')) if path.exists() else {"limit_usd": limit, "calls": [], "unknown_outcome": False}
        prior = Path(__file__).resolve().parents[1] / "data/evaluation/dimension-reanalysis-v1/api-ledger.json"
        prefix = json.loads(prior.read_text(encoding="utf-8"))["calls"]
        if self.data["calls"][:len(prefix)] != prefix:
            raise ValueError("Historical cost prefix changed")
        if self.data.get("budget_authorization") != "User approved USD 2 cumulative ceiling on 2026-09-14":
            raise ValueError("Budget authorization missing")
