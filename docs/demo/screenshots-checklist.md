...
- Trust & Safety at `/trust` showing the local-demo limitation, layered request flow, current control evidence, and visibly incomplete production-readiness checklist.
- Algorithm Quality Lab at `/dev-admin/retrieval-playground` showing named profile comparison and historical failure visibility.
...
- Capture implemented review, cleanup, extraction, and approval/indexing states only; do not imply AI cleanup indexes content automatically.
